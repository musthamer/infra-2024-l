...
run_mysql_query_or_exit active_rows "
    ...
    WHERE p.timestamp >= FROM_UNIXTIME($active_since_ts)
    ORDER BY p.ship_mmsi ASC, p.timestamp DESC;
"
...
print_json_header
printf '{"status":"success","implementation":"simple","user_email":"%s","server_time":%s,"ships":[' "$(json_escape "$user_email")" "$now_ts"

first_ship=1
last_mmsi=""
while IFS=$'\t' read -r mmsi ship_name latitude longitude timestamp; do
    [ -z "$mmsi" ] && continue
    if [ "$mmsi" = "$last_mmsi" ]; then
        continue
    fi
